...
int main()
{
    ...
    assert_near(disk_color.r, 1.0f);
    assert_near(disk_color.g, 1.0f);
    assert_near(disk_color.b, 0.0f);

    const Color antialiased_edge = renderer.RednerPixel(789, 300);
    assert(antialiased_edge.g > 0.5f);
    assert(antialiased_edge.g < 1.0f);

    const Renderer renderer_without_ssaa(800, 600, false);
    const Color aliased_edge = renderer_without_ssaa.RednerPixel(789, 300);
    assert_near(aliased_edge.r, 1.0f);
    assert_near(aliased_edge.g, 1.0f);
    assert_near(aliased_edge.b, 0.0f);
    ...
    assert_near(one_sample_edge.r, 1.0f);
    assert_near(one_sample_edge.g, 1.0f);
    assert_near(one_sample_edge.b, 0.0f);
    ...
    assert_near(triangle_color.r, 0.0f);
    assert_near(triangle_color.g, 1.0f);
    assert_near(triangle_color.b, 1.0f);

    const Color gap_color = renderer.RednerPixel(536, 300);
    assert_near(gap_color.r, 536.0f / 800.0f);
